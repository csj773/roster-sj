...
const firebasePath = path.join(process.cwd(), "firebase-service-account.json");
...
// ------------------- Google Sheets 초기화 (JSON 파일 사용) -------------------
const sheetsPath = path.join(process.cwd(), "google-sheets-credentials.json");
if (!fs.existsSync(sheetsPath)) {
  console.error("❌ google-sheets-credentials.json 파일이 없습니다.");
  process.exit(1);
}
const sheetsCredentials = JSON.parse(fs.readFileSync(sheetsPath, "utf-8"));
...
(async () => {
  ...
  if (!rosterLinkHandle) throw new Error("Roster 링크를 찾지 못했습니다.");
  const element = rosterLinkHandle.asElement();
  if (!element) throw new Error("Roster 링크 element 변환 실패");
  await Promise.all([element.click(), page.waitForNavigation({ waitUntil: "networkidle0" })]);
