...
(async () => {
  ...
  try {
    ...
    // ------------------- Roster 메뉴 이동 -------------------
    const rosterLink = await page.$x("//a[contains(text(),'Roster')]");
    if (rosterLink.length === 0) throw new Error("Roster 링크를 찾지 못했습니다.");
    await Promise.all([rosterLink[0].click(), page.waitForNavigation({ waitUntil: "networkidle0" })]);
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      // PDC 계정은 로그용으로만 저장, 필요 시
      // docData.pdc_user_name = username;

      // Activity 없으면 해당 날짜 삭제
      if (!docData.Activity || docData.Activity.trim() === "") {
        const snapshot = await db.collection("roster").where("Date", "==", docData.Date).where("userId","==",flutterflowUid).get();
        snapshot.forEach(doc => batch.delete(doc.ref));
        continue;
      }

      // Firestore batch set
      const docRef = db.collection("roster").doc();
    ...
    const spreadsheetId = process.env.SHEET_ID || "1mKjEd__zIoMJaa6CLmDE-wALGhtlG-USLTAiQBZnioc";
    const sheetName = "Roster1";
    const sheetValues = values.map((row, idx) => idx === 0 ? row : [...row.slice(0,1).map(cell=>convertDate(cell)), ...row.slice(1)]);
