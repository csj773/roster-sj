...
import { blhStrToHour, hourToTimeStr, parseUTCDate, calculateET, calculateNT, convertDate } from "./flightTimeUtils.js"; // <-- convertDate 추가
...
(async () => {
  ...
  const sheetValues = values.map((row,idx)=>{
    ...
    newRow[0] = convertDate(row[0]); // <-- flightTimeUtils에서 import
    return newRow;
  });
