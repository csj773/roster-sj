...
(async () => {
  ...
  const rosterRaw = await page.evaluate(() =>
    Array.from(document.querySelectorAll("table tr")).map(tr => Array.from(tr.querySelectorAll("td")).map(td => td.innerText.trim()))
  );
  ...
  fs.writeFileSync(path.join(publicDir,"roster.csv"),
  ...
  savePerDiemCSV(perdiemList, path.join(publicDir,"perdiem.csv"));
  await uploadPerDiemFirestore(perdiemList, flutterflowUid);

  // ------------------- Firestore Roster 업로드 -------------------
  ...
  for(let i=1;i<values.length;i++){
    const row = values[i]; const docData={};
    headers.forEach((h,idx)=>{ docData[h]=row[idx]||""; docData[headerMapFirestore[h]||h]=row[idx]||""; });
    docData.userId=flutterflowUid; docData.adminId=firestoreAdminUid; docData.pdc_user_name=username;
    if(!docData.Activity || docData.Activity.trim()==="") continue;
    docData.ET = calculateET(docData.BLH);
    if(docData.From!==docData.To){ const flightDate=new Date(convertDate(docData.Date)); docData.NT=calculateNTFromSTDSTA(docData.STDZ,docData.STAZ,flightDate); } else { docData.NT="00:00"; }
    docData.CrewArray=parseCrewString(docData.Crew);
    const {Year,Month}=parseYearMonthFromEeeDd(docData.Date); docData.Year=Year; docData.Month=Month;
    ...
    const querySnapshot = await db.collection(firestoreCollection)
      ...
      .where("Crew","==",docData.Crew).get();
    if(!querySnapshot.empty){ for(const doc of querySnapshot.docs){ await db.collection(firestoreCollection).doc(doc.id).delete(); } }
  ...
  const sheetValues = values.map((row,idx)=>{ if(idx===0) return row.slice(0,15); const newRow=[...row.slice(0,15)]; newRow[0]=convertDate(row[0]); return newRow; });
  try{ await sheetsApi.spreadsheets.values.update({ spreadsheetId, range:`${sheetName}!A1`, valueInputOption:"RAW", requestBody:{values:sheetValues} }); console.log("✅ Google Sheets 업로드 완료"); } 
