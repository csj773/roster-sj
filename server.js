...
app.post("/runRoster", async (req,res)=>{
  try{
    ...
    if(!username||!password) return res.status(400).json({error:"PDC 계정(username/password)이 입력되지 않았습니다."});
    if(!firebaseUid||!adminFirebaseUid) return res.status(400).json({error:"FlutterFlow UID 또는 Admin UID가 입력되지 않았습니다."});

    const env={...process.env, INPUT_PDC_USERNAME:username, INPUT_PDC_PASSWORD:password, INPUT_FIREBASE_UID:firebaseUid, INPUT_ADMIN_FIREBASE_UID:adminFirebaseUid };
  ...
  }catch(e){ console.error("❌ 서버 실행 에러:",e); res.status(500).json({error:e.message}); }
});
